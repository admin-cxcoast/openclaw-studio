...
export const FleetSidebar = ({
  ...
  // Close instance dropdown when clicking outside
  useEffect(() => {
    ...
    >
      {gatewayTabs && gatewayTabs.length > 1 && onGatewaySelect ? (
        <div className="relative" ref={instanceDropdownRef}>
          <p className="mb-1 px-1 font-mono text-[9px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
            Instance
          </p>
          <button
            type="button"
            className="flex w-full items-center justify-between gap-2 rounded-md border border-border bg-surface-1 px-3 py-2 text-left transition hover:bg-surface-2"
            onClick={() => setInstanceDropdownOpen((prev) => !prev)}
          >
            <div className="flex items-center gap-2 min-w-0">
              {gatewayTabs[activeGatewayIdx]?.status === "running" && (
                <span className="h-2 w-2 shrink-0 rounded-full bg-green-500" />
              )}
              <span className="truncate font-mono text-xs font-semibold text-foreground">
                {gatewayTabs[activeGatewayIdx]?.name || gatewayTabs[activeGatewayIdx]?.primaryAgentName || `Port ${gatewayTabs[activeGatewayIdx]?.port}`}
              </span>
              <span className="font-mono text-[9px] text-muted-foreground">
                {agents.length} {agents.length === 1 ? "agent" : "agents"}
              </span>
            </div>
            <span className="font-mono text-[10px] text-muted-foreground">{instanceDropdownOpen ? "\u25B4" : "\u25BE"}</span>
          </button>
          {instanceDropdownOpen && (
            <div className="absolute left-0 right-0 top-full z-50 mt-1 max-h-64 overflow-auto rounded-lg border border-border bg-card shadow-lg">
              {gatewayTabs.map((gw, idx) => {
                const active = idx === activeGatewayIdx;
                const count = active ? agents.length : (gw.agentCount ?? 1);
                return (
                  <button
                    key={gw.instanceId}
                    type="button"
                    ...
                    } ${idx === 0 ? "rounded-t-lg" : ""} ${idx === gatewayTabs.length - 1 ? "rounded-b-lg" : ""}`}
                    onClick={() => {
                      onGatewaySelect(idx);
                      setInstanceDropdownOpen(false);
                    }}
                  >
                    ...
                    <span className="flex-1 truncate font-mono text-[11px] font-semibold">{gw.name || gw.primaryAgentName || `Port ${gw.port}`}</span>
                    <span className="font-mono text-[9px] text-muted-foreground">
